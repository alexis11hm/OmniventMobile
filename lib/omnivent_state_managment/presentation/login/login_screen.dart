             ...
             Center(
               child: InkWell(
                 onTap: (){
                   ...
                   loginService.iniciarSesion(loginModel).then(
                      (value) => {
                        if(value.estatus == 200){
                          secure.escribirValorAlmacenamiento(almacenamiento,'token','Bearer ${value.respuesta}'),
                          Navigator.of(context).pushReplacement(CupertinoPageRoute(builder: (_) => HomeScreen()))
                        }else{
                          print('${value.estatus}, ${value.respuesta}'),
    ...
    return Positioned(
      ...
      left: (size / 2) - (sizeContainer / 2),
        ...
        child: Column(
              ...
              SizedBox(height: 30,),
              SvgPicture.asset('assets/login/login.svg',width: 180,height: 180,),
              ]
        ),
      ),
    );
