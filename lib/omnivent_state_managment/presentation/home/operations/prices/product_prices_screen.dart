...
class __ScaffoldState extends State<_Scaffold> {
  ...
  Widget build(BuildContext context) {
    ...
    buscarInformacion(String buscar){
      ...
      final List<PrecioModel> listaPrecios = preciosProvider.preciosBuscar.where(
                          (precio) => 
                          precio.lipDetConIva.toString().toLowerCase().contains(buscar.toLowerCase()) ||
                          precio.lipDetSinIva.toString().toLowerCase().contains(buscar.toLowerCase()) ||
                          precio.lipId.toString().toLowerCase().contains(buscar.toLowerCase()) ||
                          precio.listaPrecio.toLowerCase().contains(buscar.toLowerCase()) ||
                          precio.proId.toString().toLowerCase().contains(buscar.toLowerCase()) ||
                          precio.productoDescripcion.toLowerCase().contains(buscar.toLowerCase())
                          ).toList();
                    ...
                    return Dismissible(
                        ...
                        children: [
                          ...
                          onTap: () => Navigator.of(context).push(
                            CupertinoPageRoute(
                              builder: (BuildContext context) => ProductsSalesDetailsScreen())),
                              leading: FadeInImage(
                          ...
                          subtitle: Column(
                            ...
                            children: [
                              Text('${precios[index].listaPrecio } \nCB: ${(precios[index].productoCodigoBarras != null) ? precios[index].productoCodigoBarras : 'Sin Codigo'}'),
                              SizedBox(height: 3),
                              Row(
                                children: [
                                  Text('c/i: \$${precios[index].lipDetConIva.toStringAsFixed(2)}'),
                                  SizedBox(width: 10),
                                  Text('s/i: \$${precios[index].lipDetSinIva.toStringAsFixed(2)}'),
                                ],
                              ),
                            ],
                          ),
