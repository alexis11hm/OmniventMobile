    ...
    cargarInformacion() {
      almacenamiento.read(key: 'token').then((token) => {
            almacenamiento.read(key: 'rutaAPI').then((ruta) => {
                  ventasDetalleService.ObtenerVentasDetalle(token, ruta)
                      .then((respuesta) => {
                            if (respuesta.estatus == 200)
                              {
                                ...
                                print(
                                    'Ventas Detalle: ${ventasDetalleProvider.ventasDetalleBuscar.length}')
                              }
...
class _ContentSales extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
        child: (ventasDetalle != null || ventasDetalle.length != 0)
            ? Column(
                children: [
                  ListView.builder(
                    shrinkWrap: true,
