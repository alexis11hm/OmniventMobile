...
class ProductsSalesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ChangeNotifierProvider(
      create: (_) => new VentasDetalleProvider(),
      child: _Scaffold());
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    cargarInformacion(){
      almacenamiento.read(key: 'token').then((token) => {
          ventasDetalleService.ObtenerVentasDetalle(token).then((respuesta) => {
              if(respuesta.estatus == 200){
                print('Ventas Detalle'),
                ventasDetalleProvider.ventasDetalle = respuesta.respuesta,
                ventasDetalleProvider.ventasDetalleBuscar = respuesta.respuesta,
    ...
    buscarInformacion(String buscar){
      ...
      final List<VentaDetalleModel> listaVentasDetalle = ventasDetalleProvider.ventasDetalleBuscar.where(
                          (ventaDetalle) => 
                          ventaDetalle.producto.toLowerCase().contains(buscar.toLowerCase()) ||
                          ventaDetalle.venta.toLowerCase().contains(buscar.toLowerCase()) ||
                          ventaDetalle.vedDescuento.toString().toLowerCase().contains(buscar.toLowerCase()) ||
                          ventaDetalle.vedPrecio.toString().toLowerCase().contains(buscar.toLowerCase()) ||
                          ventaDetalle.vedCantidad.toString().toLowerCase().contains(buscar.toLowerCase())
                          ).toList();
    ...
    return Scaffold(
        ...
        body: Stack(
          children: [
            ...
            child: _ContentSales(ventasDetalle: ventasDetalleProvider.ventasDetalle)
            ),
                ...
                return Dismissible(
                    ...
                    children: [
                      ...
                      onTap: () => Navigator.of(context).push(
                        CupertinoPageRoute(
                          builder: (BuildContext context) => ProductsSalesDetailsScreen(ventaDetalle: ventasDetalle[index],))),
                      leading: Hero(
                          tag: '${ventasDetalle[index].vedId+ventasDetalle[index].vtaId}',
                          child: FadeInImage(
                      ...
                      subtitle: Column(
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              Text('Precio: \$${ventasDetalle[index].vedPrecio}'),
                              SizedBox(width: 10),
                              Text('Cantidad: ${ventasDetalle[index].vedCantidad}'),
                            ],
                          ),
                        ],
                      ),
                      ...
                      child: Padding(
                        ...
                        child: Scaffold(
                            body: SingleChildScrollView(
                              child: Column(
                              children: [
                                Text('Filtrar Ventas de Productos',
                                style: TextStyle(
                                  fontSize: 20,
                                ...
                                Container(
                                  ...
                                  child: DropdownButton(
                                    items: <DropdownMenuItem>[
                                      DropdownMenuItem(child: Text('Almacenes la Mexicana')),
                                ...
                                Container(
                                  ...
                                  items: <DropdownMenuItem>[
                                    DropdownMenuItem(
                                      child: Text('P.V. por Fecha, F. Venta y Corte')),
                                    DropdownMenuItem(
