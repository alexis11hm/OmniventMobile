...
class ProductosService{
  ...
  Future<ResponseModel> ObtenerProductos(String token,String urlBase) async{
      try{
          Response response;
          Dio dio = new Dio();
  ...
  Future<ResponseModel> ObtenerFamilias(String token,String urlBase) async{
      try{
          Response response;
          Dio dio = new Dio();
          ...
          dio.options.headers["authorization"] = token;
          dio.options.baseUrl = urlBase;
          response = await dio.get('Productos/Familias');
              
          if(response != null){
            if(response.statusCode == 200){
              final List<dynamic> listaFamilias = response.data;
              if(listaFamilias == null) return ResponseModel(estatus: 200,respuesta: []);
              return ResponseModel(
                estatus: 200,
                respuesta: listaFamilias
              );
            }else{
                return ResponseModel(
                  estatus: response.statusCode,
